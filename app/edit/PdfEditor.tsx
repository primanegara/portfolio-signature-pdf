...
import { Trash2, Download, Zap, Edit3, Image as ImageIcon, ZoomIn, ZoomOut, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function EditPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
            <div className="flex-1 flex flex-col h-screen overflow-hidden">
                ...
                <div className="h-16 bg-white border-b flex items-center justify-between px-6 shrink-0">
                    <div className="flex items-center space-x-4">
                        <h2 className="font-semibold text-slate-800 truncate max-w-xs">{pdfFile.name}</h2>
                    ...
                    <div className="flex items-center space-x-2">
                        <Button variant="outline" size="sm" onClick={() => setPageNumber(Math.max(pageNumber - 1, 1))} disabled={pageNumber <= 1}>
                            <ChevronLeft className="h-4 w-4" />
                        </Button>
                        <span className="text-sm">Page {pageNumber} of {numPages}</span>
                        <Button variant="outline" size="sm" onClick={() => setPageNumber(Math.min(pageNumber + 1, numPages))} disabled={pageNumber >= numPages}>
                            ...
                            >
                                <div className="relative w-full h-full">
                                    ...
                                    <button
                                        ...
                                        className="absolute -top-3 -right-3 bg-red-500 text-white rounded-full p-1 hidden group-hover:block"
                                    >
            ...
            {/* Right Sidebar */}
            <div className="w-full md:w-80 bg-white border-l flex flex-col h-screen shrink-0">
                <div className="p-6 border-b">
                    <h3 className="text-lg font-semibold flex items-center gap-2">
                        <Zap className="h-5 w-5 text-blue-500" />
                        Sign Document
                    </h3>
                    <p className="text-sm text-slate-500 mt-1">Add signatures to your PDF</p>
                </div>

                <div className="p-6 space-y-4 flex-1">
                    <Dialog open={isDrawOpen} onOpenChange={setIsDrawOpen}>
                        <DialogTrigger asChild>
                            <Button variant="outline" className="w-full h-24 flex flex-col gap-2 border-dashed border-2 hover:border-blue-500 hover:bg-blue-50">
                        ...
                        <DialogContent className="sm:max-w-md">
                            ...
                            <div className="border-2 rounded-lg overflow-hidden bg-white">
                                <SignatureCanvas
                                    ref={signatureRef}
                                    canvasProps={{ className: "w-full h-64 bg-slate-50", style: { width: "100%", height: "256px" } }}
                                />
                            </div>
                    ...
                    <Dialog open={isImageOpen} onOpenChange={setIsImageOpen}>
                        <DialogTrigger asChild>
                            <Button variant="outline" className="w-full h-24 flex flex-col gap-2 border-dashed border-2 hover:border-blue-500 hover:bg-blue-50" onClick={() => setIsImageOpen(true)}>
                        ...
                        <DialogContent className="sm:max-w-md">
                            ...
                            <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed rounded-lg bg-slate-50 hover:bg-slate-100 cursor-pointer" onClick={() => imageUploadRef.current?.click()}>
                                <ImageIcon className="h-12 w-12 text-slate-400 mb-4" />
                                <p className="text-sm font-medium">Click to upload signature</p>
                                <p className="text-xs text-slate-500">Supports PNG, JPG (Transparent PNG recommended)</p>
                                <input
                                    type="file"
                                    ref={imageUploadRef}
                                    className="hidden"
